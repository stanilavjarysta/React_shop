...
import {setWeapon, updateMoney, removeFromInventory} from "../features/trade";
...
const Inventory = () => {
    ...
    const arr = [...Array(char.inventorySlots - inventory.length).keys()]
    ...
    function identifyItem(item, index) {
        if(item.maxDamage) {
            return <div key={index} className="itemSlot d-flex" onClick={() => action('weapon', item, index)}>
                <img src={item.image} alt=""/>
                <div className="grow1 d-flex column j-center a-center">
                    <div>Max Damage: {item.maxDamage}</div>
                    <div>Price: {item.price}</div>
                    <div>Energy cost: {item.energyPerHit}</div>
                </div>
        ...
        if(item.effect) {
            return <div key={index} className="itemSlot d-flex" onClick={() => action('potion', item, index)}>
                <img src={item.image} alt=""/>
                <div className="grow1 d-flex column j-center a-center">
        ...
        return <div key={index} className="itemSlot d-flex" onClick={() => action('item', item, index)}>
            <img src={item.image} alt=""/>
            <div className="grow1 d-flex column j-center a-center">
                <div>Price: {item.price}</div>
            </div>
    ...
    return (
        <div className="inventory grow1">

            {inventory.map((x, i) => identifyItem(x, i))}

            {arr.map((x, i) => <div key={i} className="itemSlot"></div>)}
        </div>
    );
};
