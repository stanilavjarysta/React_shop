...
import DropItems from "../components/Items";
...
const ArenaWindow = () => {
    const [getMonster, setMonster] = useState(15)
    ...
    if(getMonsterHealth <= 0) {
        ...
        setGameState(2)
        const random = (num) => Math.round(Math.random()*num)
        const itemsDropped = random(monsters[getMonster].maxItemsDrop)
    ...
    return (
        <div className="d-flex space-btw">
            {getGameState === 2 && <div className="grow1"><Inventory/></div> }
            {getGameState !== 2 && <Arena player={true} item={char} gameState={getGameState} healthProgress={getPlayerHealth} energyProgress={getPlayerEnergy}/>}
            ...
            {getGameState !== 2 && <Arena player={false} item={monsters[getMonster]} healthProgress={getMonsterHealth} energyProgress={100} gameState={getGameState}/>}
        </div>
    );
};
